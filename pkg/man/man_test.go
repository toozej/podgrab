...
func TestNewManCmd(t *testing.T) {
	...
	expectedShort := "Generates podgrab's command line manpages"
...
func TestNewManCmd_RunE(t *testing.T) {
	...
	if !strings.Contains(output, "testroot") {
		t.Errorf("expected man page output to contain root command name 'testroot', got: %q", output[:minInt(len(output), 200)])
	}
}
...
func TestNewManCmd_RunEWithSubcommands(t *testing.T) {
	...
	if !strings.Contains(output, "sub") {
		t.Errorf("expected man page output to mention subcommand 'sub', got: %q", output[:minInt(len(output), 500)])
	}
}
...
func TestNewManCmd_RunEWithRootFlags(t *testing.T) {
	...
	if !strings.Contains(output, "flagapp") {
		t.Errorf("expected man page output to contain 'flagapp', got: %q", output[:minInt(len(output), 200)])
	}
}
